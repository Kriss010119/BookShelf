                            ...
                            {searchResults.map((book) => (
                                ...
                                >
                                    {book.formats['image/jpeg'] ? (
                                        ...
                                        <div className={styles.bookInitial}>
                                            {book.title.charAt(0)}
                                        </div>
                                    )}
                                    <div className={styles.bookInfo}>
                                        <h3 className={styles.bookTitle}>{book.title}</h3>
                                        <p className={styles.bookAuthors}>by {book.authors.map((a: { name: any; }) => a.name).join(', ')}</p>
                                        {book.download_count && <p className={styles.bookDownloads}>Downloads: {book.download_count.toLocaleString()}</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
