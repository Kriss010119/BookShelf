...
const Library = ({isPublic = false}) => {
    ...
    const {shelves, books} = isPublic ?
        ...
        libraryState;
    const ownerInfo = libraryState.ownerInfo;
    ...
    const bookDataMap = books ? books.reduce((acc, book) => {
            ...
            <div className={styles.header}>
                <div className={styles.imgAndTitle}>
                    {isPublic && (
                        <img
                            /* @ts-ignore  */
                            src={`/img/profile/${ownerInfo.avatarImage}`}
                            alt="Profile"
                            className={styles.avatarImage}
                        />
                    )}
                    <h1 className={styles.title}>{isPublic && ownerInfo ? `${ownerInfo.username}'s Library` : 'My Library'}</h1>
                </div>
                {!isPublic && (
                    <button
